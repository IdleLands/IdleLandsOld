...
class Player extends Character
  ...
  pickRandomTile: ->
    ...
    switch
      ...
      when 81 <=  randomDir <=  84      then dir = ((@lastDir + 1) % 8 + 8) % 8
      when 85 <=  randomDir <=  87      then dir = ((@lastDir + 2) % 8 + 8) % 8
      when randomDir = 88               then dir = ((@lastDir + 3) % 8 + 8) % 8
      when randomDir = 89               then dir = ((@lastDir + 4) % 8 + 8) % 8
      when randomDir = 90               then dir = ((@lastDir + 5) % 8 + 8) % 8
      when 91 <=  randomDir <=  93      then dir = ((@lastDir + 6) % 8 + 8) % 8
      when 94 <=  randomDir <=  97      then dir = ((@lastDir + 7) % 8 + 8) % 8
    else dir = 9
